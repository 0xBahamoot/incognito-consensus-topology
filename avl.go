...
func (root *TreeNode) searchRootOfNode(key interface{}) *TreeNode {
	//root of the tree
	if root.key == key {
		return root
	}
	...
	if root.issmaller(key, root.key) {
		...
		return root.left.searchRootOfNode(key)
	}
	...
	return root.right.searchRootOfNode(key)
}
...
func (t *AVLTree) Insert(key interface{}, value interface{}, issmaller Comparable) (new *TreeNode, err error) {
	if t == nil {
...
func (t *AVLTree) Exists(key interface{}) bool {
	if t == nil {
		return false
	}
...
func (t *AVLTree) Intersection(x *AVLTree) (res *AVLTree) {
	if t == nil || x == nil {
		return nil
	}
	res = newAVLTree()
...
func Intersection(candidates []*AVLTree) (res *AVLTree) {
	n := len(candidates)
	if n == 0 {
		return nil
	}
	...
	chA := make([]chan *AVLTree, n/2)
	chB := make([]chan *AVLTree, n/2)
	chRet := make([]chan *AVLTree, n/2)

	// Start a go routine that builds intersection of each pair of candidates
	for i := 0; i < n/2; i++ {
		chA[i] = make(chan *AVLTree)
		chB[i] = make(chan *AVLTree)
		chRet[i] = make(chan *AVLTree)
		go func(chA chan *AVLTree, chB chan *AVLTree, chRes chan *AVLTree) {
			a := <-chA
			b := <-chB
			if a == nil || b == nil {
				chRes <- nil
...
func (t *AVLTree) Each(f EachFunc, vals ...interface{}) {
	if t == nil {
		return
	}
...
func (t *AVLTree) Dump() (res []interface{}) {
	if t == nil {
		return
	}
...
func (t *AVLTree) TopN(n int) (res []interface{}) {
	if t == nil {
		return
	}
